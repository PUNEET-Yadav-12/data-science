...
import joblib
import laptopdata  # Import the database module

# Load the model
model = joblib.load('laptop price/Random_Regression.lb')

app = Flask(__name__)

# Initialize the database (create tables if they don't exist)
laptopdata.create_table()
...
def predict():
    if request.method == "POST":
        try:
            ...
            laptopdata.insert_laptop_data(screen_size, ram, weight, processor_speed, width, height, storage_capacity, gpu_code, company, type_, os, generation)
            ...
            ] + company_encoded + type_encoded + os_encoded + generation_encoded]
